...
const main = async () => {
  console.log(
    "🚀 Starting Vector Search Experiment with Turso + Drizzle + HuggingFace",
  );
  ...
  console.log(
    `✅ Generated ${docVectors.length} document embeddings (${EMBEDDING_DIMENSIONS} dimensions each)`,
  );
  ...
  for (const queryText of queries) {
    ...
    results.rows.forEach((row, idx) => {
      ...
      console.log(
        `  ${idx + 1}. [sim: ${similarity.toFixed(4)}] ${row.content}`,
      );
    });
  }
  ...
  const scores = await matmul(
    sentence_embedding,
    sentence_embedding.transpose(1, 0),
  );
  ...
  ranking.slice(0, 3).forEach((item, idx) => {
    console.log(
      `  ${idx + 1}. [sim: ${item.score.toFixed(4)}] ${item.content}`,
    );
  });

  // Database ranking
  const queryTokens2 = await tokenizer([prefixes.query + testQuery], {
    padding: true,
  });
